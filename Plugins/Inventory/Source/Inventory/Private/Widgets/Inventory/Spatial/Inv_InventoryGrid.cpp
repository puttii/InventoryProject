...
FInv_SpaceQueryResult UInv_InventoryGrid::CheckHoverPosition(const FIntPoint& Position, const FIntPoint& Dimensions)
{
	...
	// 比如一个物品2×2，占了四个格子，需要判断每个格子的 upper left index是否相同以此判断是不是一个物品
	...
	if (OccupiedUpperLeftIndices.Num() == 1) // 只有一个物品，可以交换/合并
	{
...
void UInv_InventoryGrid::UnHighlightSlots(const int32 Index, const FIntPoint& Dimensions)
{
	UInv_InventoryStatics::ForEach2D(GridSlots, Index, Dimensions, Columns, [&](UInv_GridSlot* GridSlot)
	{
		...
		{	// 如果这个 slot上有物品，那么不会 SetOccupiedTexture
...
FInv_SlotAvailabilityResult UInv_InventoryGrid::HasRoomForItem(const FInv_ItemManifest& Manifest, const int32 StackAmountOverride)
{
	...
	{
		// If we don't have anymore to fill, 提前退出
...
{
	// 有三种 inventory grid， 只有和 item的类别对的上的才需要接着往下执行
...
void UInv_InventoryGrid::OnSlottedItemClicked(int32 GridIndex, const FPointerEvent& MouseEvent)
{
	...
	// hovered item和 clicked inventory item是否为一种物品? 他们是否可以堆叠?
	if (IsSameStackable(ClickedInventoryItem))
	{
		...
		// 是否交换 swap counts (Room in the clicked slot == 0 && HoveredStackCount < MaxStackSize)
...
{
	if (!IsValid(HoverItem)) return;
	// ItemDropIndex 是拖动 hover item时候左上角的索引
